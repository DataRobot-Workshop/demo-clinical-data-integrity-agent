...
#
# The Programmatic Access Token is a secret, so it goes through the platform's
# normal runtime credential path (Shared Secure Configuration) exactly like
# SESSION_SECRET_KEY above — registered as a credential-type runtime parameter,
# never a plaintext value in code. At runtime datarobot-genai resolves it back
# to the SNOWFLAKE_PAT environment variable the agent's Config reads.
SNOWFLAKE_PAT_PARAM: Final[str] = "SNOWFLAKE_PAT"
if snowflake_pat := os.environ.get(SNOWFLAKE_PAT_PARAM):
    snowflake_pat_cred = pulumi_datarobot.ApiTokenCredential(
        agent_asset_name + " Snowflake PAT",
        args=pulumi_datarobot.ApiTokenCredentialArgs(api_token=str(snowflake_pat)),
    )
    agent_runtime_parameter_values.append(
        pulumi_datarobot.CustomModelRuntimeParameterValueArgs(
            type="credential",
            key=SNOWFLAKE_PAT_PARAM,
            value=snowflake_pat_cred.id,
        ),
    )
    pulumi.info("Configured with SNOWFLAKE_PAT credential")

# Non-secret Snowflake connection settings — passed through as plain string
# runtime parameters when set in the environment. Defaults live in agent Config.
for _sf_key in (
    "SNOWFLAKE_ACCOUNT",
    "SNOWFLAKE_USER",
    "SNOWFLAKE_WAREHOUSE",
    ...
    if _sf_val := os.environ.get(_sf_key):
        agent_runtime_parameter_values.append(
            pulumi_datarobot.CustomModelRuntimeParameterValueArgs(
                key=_sf_key,
                type="string",
