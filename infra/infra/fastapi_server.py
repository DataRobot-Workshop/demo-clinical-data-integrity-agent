...
# Clinical Data Integrity Agent — Snowflake connection for the FastAPI backend.
# The backend serves the encounters list, detail, audit trail, approvals, and
# reset by connecting to Snowflake DIRECTLY (not through the agent), so these
# credentials must exist in the APP SOURCE runtime environment — separate from
# the agent deployment's copy. Without them the app returns 502 on /encounters.
snowflake_app_runtime_parameters: list[
    pulumi_datarobot.ApplicationSourceRuntimeParameterValueArgs
] = []
if snowflake_pat := os.environ.get("SNOWFLAKE_PAT"):
    snowflake_pat_cred = pulumi_datarobot.ApiTokenCredential(
        f"Agentic Application Starter Snowflake PAT [{PROJECT_NAME}]",
        args=pulumi_datarobot.ApiTokenCredentialArgs(api_token=str(snowflake_pat)),
    )
    snowflake_app_runtime_parameters.append(
        pulumi_datarobot.ApplicationSourceRuntimeParameterValueArgs(
            type="credential",
            key="SNOWFLAKE_PAT",
    ...
    pulumi.info("FastAPI app configured with SNOWFLAKE_PAT credential")
...
fastapi_server_app_runtime_parameters: list[
    ...
    + memory_runtime_parameters
    + snowflake_app_runtime_parameters
    + [
        parameter
